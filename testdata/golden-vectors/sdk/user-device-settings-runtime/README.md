...
`settings-runtime-readiness.json` names the shared-contract behavior-family gate for this runtime slice: the settings path, read success, read-failure no-write behavior, read-then-write payload planning, write-failure-after-payload terminals, daylight-saving payload shape, protobuf field preservation, public facade error mapping, platform facade vector references, and the `compile-verification-gate` must remain covered before production settings runtime planning moves into shared code.
