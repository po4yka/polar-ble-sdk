...
- [x] Promote command and disk/time facade runtime planning to shared KMP code.
...
| Promote command and disk/time facade runtime planning to shared KMP code. | `sources/Android/android-communications/shared/src/commonMain/kotlin/com/polar/shared/runtime/PolarRuntimeOrchestration.kt` owns deterministic command and disk/time facade planning for H10 recording queries, reset notification flags, sync notification sequences, disk-space/local-time queries, V2 system/local-time sequencing, H10 single-query local-time behavior, and transport-error/platform-split terminals; `sources/Android/android-communications/shared/src/commonTest/kotlin/com/polar/sharedtest/CommandRuntimePolicyCommonTest.kt` and `sources/Android/android-communications/shared/src/commonTest/kotlin/com/polar/sharedtest/DiskTimeRuntimePolicyCommonTest.kt` now exercise production common planning code against `testdata/golden-vectors/sdk/command-runtime/reset-sync-h10-command-policy.json`, `testdata/golden-vectors/sdk/command-runtime/reset-sync-h10-command-readiness.json`, `testdata/golden-vectors/sdk/disk-time-runtime/disk-time-query-policy.json`, and `testdata/golden-vectors/sdk/disk-time-runtime/disk-time-query-readiness.json`; platform BLE transport execution, protobuf payload builders, filesystem capability gates, sync error compatibility, and public facade error mapping remain platform-owned until the next runtime delegation slice pins adapter compatibility. |
...
|---|---|
| Remaining production consumption of shared code. | Android now consumes `:shared` for the Device ID/UUID, time/date, product capability lookup, type utility, PMD settings/control-point parsing, ECG/ACC/GYR/MAG/PPG/PPI/pressure/temperature/skin-temperature sensor parsers, disk-space and skin-temperature SDK model mapping slices, offline-recording metadata, training-session reference discovery, and firmware utility mappings. Shared KMP now owns command and disk/time runtime planning, while Android/iOS facade adapters still own BLE transport execution, protobuf payload builders, filesystem gates, public error mapping, and platform split compatibility. iOS Xcode/CocoaPods production targets now link `PolarBleSdkShared.framework` and delegate Device ID/UUID plus selected time/date utility behavior when the framework is available, while SwiftPM/watchOS keep fallback Swift code until binary artifact packaging is defined. Byte-level training-session protobuf/gzip parsing, firmware workflow execution, and every other Android/iOS behavior family remain open until each slice adds failing common tests, platform facade compatibility tests, and a rollback path. |
