...
- Android production consumes shared KMP through `implementation(project(":shared"))` in `sources/Android/android-communications/library/build.gradle.kts`; Android local AAR distribution uses the documented two-AAR compatibility model with `polar-ble-sdk.aar` plus `polar-ble-sdk-shared.aar`.
