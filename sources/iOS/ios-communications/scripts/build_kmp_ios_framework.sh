...
CURRENT_ARCH=${CURRENT_ARCH:-${ARCHS%% *}}
...
case "$PLATFORM_NAME:$CURRENT_ARCH" in
    ...
    iphonesimulator:x86_64)
        KMP_TARGET=IosX64
        FRAMEWORK_SOURCE="$GRADLE_ROOT/shared/build/bin/iosX64/${CONFIGURATION}Framework/PolarBleSdkShared.framework"
        ;;
    iphonesimulator:*)
...
if [ -f "$FRAMEWORK_STAMP" ] && [ "${POLAR_BLE_SDK_FORCE_KMP_FRAMEWORK_BUILD:-}" != "1" ]; then
