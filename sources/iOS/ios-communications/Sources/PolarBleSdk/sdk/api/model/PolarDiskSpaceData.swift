    ...
    static func fromProto(proto: Protocol_PbPFtpDiskSpaceResult) -> PolarDiskSpaceData {
        ...
        if let sharedInput = SharedDiskSpaceInput(proto: proto) {
            return PolarDiskSpaceData(
                totalSpace: UInt64(PolarIosSharedBridge.shared.diskSpaceTotalSpace(fragmentSize: sharedInput.fragmentSize, totalFragments: sharedInput.totalFragments, freeFragments: sharedInput.freeFragments)),
                freeSpace: UInt64(PolarIosSharedBridge.shared.diskSpaceFreeSpace(fragmentSize: sharedInput.fragmentSize, totalFragments: sharedInput.totalFragments, freeFragments: sharedInput.freeFragments))
            )
        }
        #endif
        return PolarDiskSpaceData(
