...
class BDBleApiImpl private constructor(context: Context, features: Set<PolarBleSdkFeature>) : PolarBleApi(features), BlePowerStateChangedCallback, PolarTrainingSessionApi,
    ...
    override suspend fun getLocalTime(identifier: String): LocalDateTime {
        ...
        val plan = PolarRuntimePlannerAdapter.planDiskTimeQuery("get-local-time", "GET_LOCAL_TIME")
        return try {
            val result = client.query(PolarRuntimePlannerAdapter.queryValue(plan), null)
    ...
    override suspend fun getLocalTimeWithZone(identifier: String): ZonedDateTime {
        ...
        val plan = PolarRuntimePlannerAdapter.planDiskTimeQuery("get-local-time-with-zone", "GET_LOCAL_TIME")
        return try {
            val result = client.query(PolarRuntimePlannerAdapter.queryValue(plan), null)
            val dateTime = PftpRequest.PbPFtpSetLocalTimeParams.parseFrom(result.toByteArray())
    ...
    override suspend fun startRecording(
        ...
        try {
            client.query(
                PolarRuntimePlannerAdapter.queryValue(plan),
                params.toByteArray()
            )
        } catch (throwable: Throwable) {
    ...
    override suspend fun stopRecording(identifier: String) {
        ...
        val client = session.fetchClient(BlePsFtpUtils.RFC77_PFTP_SERVICE) as BlePsFtpClient?
            ?: throw PolarServiceNotAvailable()
        val plan = PolarRuntimePlannerAdapter.planCommandQuery("h10-stop-recording", "REQUEST_STOP_RECORDING")
        try {
            client.query(PolarRuntimePlannerAdapter.queryValue(plan), null)
        } catch (throwable: Throwable) {
            throw handleError(throwable)
        }
    }
    ...
    override suspend fun requestRecordingStatus(identifier: String): androidx.core.util.Pair<Boolean, String> {
        ...
        val client = session.fetchClient(BlePsFtpUtils.RFC77_PFTP_SERVICE) as BlePsFtpClient?
            ?: throw PolarServiceNotAvailable()
        val plan = PolarRuntimePlannerAdapter.planCommandQuery("h10-recording-status", "REQUEST_RECORDING_STATUS")
        return try {
            val byteArrayOutputStream = client.query(PolarRuntimePlannerAdapter.queryValue(plan), null)
    ...
    override suspend fun fetchExercise(
        ...
        val requestBytes = PolarRuntimePlannerAdapter.fileOperationBytes(fetchOperation)

        return try {
            val byteArrayOutputStream = client.request(requestBytes)
    ...
    private fun buildPftpGetRequest(path: String): ByteArray {
        ...
        return PolarRuntimePlannerAdapter.fileOperationBytes(readOperation)
    }
    ...
    private suspend fun getSubRecordingAndOtherFilesCount(
        ...
        return try {
            ...
            val requestBytes = PolarRuntimePlannerAdapter.fileOperationBytes(readOperation)
    ...
    override suspend fun getDiskSpace(identifier: String): PolarDiskSpaceData {
        ...
        val client = session.fetchClient(BlePsFtpUtils.RFC77_PFTP_SERVICE) as BlePsFtpClient?
            ?: throw PolarServiceNotAvailable()
        val plan = PolarRuntimePlannerAdapter.planDiskTimeQuery("get-disk-space", "GET_DISK_SPACE")
        return try {
            val result = client.query(PolarRuntimePlannerAdapter.queryValue(plan), null)
    ...
    override suspend fun doFirstTimeUse(identifier: String, ftuConfig: PolarFirstTimeUseConfig) {
        ...
        val ftuRequestBytes = PolarRuntimePlannerAdapter.fileOperationBytes(ftuOperation)
        ...
        val userIdRequestBytes = PolarRuntimePlannerAdapter.fileOperationBytes(userIdOperation)
    ...
    override suspend fun getUserPhysicalConfiguration(identifier: String): PolarPhysicalConfiguration? {
        ...
        return try {
            ...
            val response = client.request(PolarRuntimePlannerAdapter.fileOperationBytes(readOperation))
    ...
    private suspend fun checkFirmwareUrlAvailabilityForUpdate(client: BlePsFtpClient, identifier: String): Triple<String?, String?, FirmwareUpdateStatus> {
        ...
        for (delayMillis in PolarRuntimePlannerAdapter.firmwareRetryDelaysMillis(maxRetries = 2)) {
            if (!PolarFirmwareUpdateUtils.isRetryableFirmwareAvailabilityFailure(result.third)) {
                return result
            }
    ...
    private suspend fun checkFirmwareUrlAvailability(client: BlePsFtpClient, identifier: String): Triple<String?, String?, FirmwareUpdateStatus> {
        return PolarFirmwareUpdateUtils.checkFirmwareUrlAvailability(client, identifier, firmwareUpdateApiFactory())
    }
    ...
    private suspend fun writeFirmwareToDevice(collector: FlowCollector<FirmwareUpdateStatus>,
        ...
        for (firmwareFile in firmwareFiles) {
            ...
            try {
                ...
                val mappedError = PolarFirmwareUpdateUtils.firmwareWriteFailure(error, firmwareFile.first)
                if (mappedError == null) {
    ...
    override suspend fun deleteStoredDeviceData(identifier: String, dataType: PolarStoredDataType, until: LocalDate?) {
        ...
        try {
            ...
            BleLogger.e(TAG, "Error while trying to delete offline recordings from device $identifier, error: $error")
            if (dataType.type != PolarStoredDataType.SDLOGS.type) {
                throw error
            }
        }
    }
    ...
    override suspend fun sendInitializationAndStartSyncNotifications(identifier: String): Boolean {
        ...
        return try {
            ...
            val client = session.fetchClient(BlePsFtpUtils.RFC77_PFTP_SERVICE) as BlePsFtpClient?
                ?: return false
            val plan = PolarRuntimePlannerAdapter.planCommandSyncStart("sync-start-success")
            client.query(PolarRuntimePlannerAdapter.queryValue(plan), null)
