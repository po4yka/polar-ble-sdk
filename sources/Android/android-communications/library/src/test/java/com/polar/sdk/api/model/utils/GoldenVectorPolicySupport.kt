...
internal fun JsonObject.hasSharedOwnershipRationale(): Boolean {
    ...
    if (expected.has("sharedOwnership")) return true
...
internal fun String.hasSharedOwnershipContext(): Boolean {
    val lower = lowercase()
    return (lower.contains("common") || lower.contains("shared")) && SHARED_OWNERSHIP_README_TERMS.any { lower.contains(it) }
}

internal fun String.hasSharedOwnershipGateLanguage(): Boolean {
    val lower = lowercase()
    return PARTIAL_SHARED_OWNERSHIP_GATE_TERMS.any { lower.contains(it) }
}
