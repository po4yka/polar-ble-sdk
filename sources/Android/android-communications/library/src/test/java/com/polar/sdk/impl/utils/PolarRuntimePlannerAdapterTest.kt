...
class PolarRuntimePlannerAdapterTest {
    ...
    @Test
    fun `available data types readiness manifest is pinned before availability shared ownership`() {
        ...
        Assert.assertEquals("Declared because this vector is consumed by shared commonTest and platform adapter tests before available-data-types runtime delegation moves further into shared.", prototype.get("reason").asString)
    ...
    fun `shared training session reconstruction plan preserves Android generated public model fields through adapter`() {
        ...
        val summaryPayload = TrainingSession.PbTrainingSession.newBuilder()
            ...
            .setModelName("Android 360")
            .setDuration(Types.PbDuration.newBuilder().setSeconds(123))
            .setDistance(12.5f)
            .setCalories(88)
            .build()
            .toByteArray()
        ...
        Assert.assertEquals("Android 360", sessionSummaryEntry.parsedPayload.modelName)
        Assert.assertEquals(123, sessionSummaryEntry.parsedPayload.durationSeconds)
        Assert.assertEquals(12, sessionSummaryEntry.parsedPayload.distanceMeters)
        Assert.assertEquals(88, sessionSummaryEntry.parsedPayload.calories)
        Assert.assertEquals("android-public-device", sessionSummary.deviceId)
        Assert.assertEquals("Android 360", sessionSummary.modelName)
        Assert.assertEquals(123, sessionSummary.duration.seconds)
        Assert.assertEquals(12.5f, sessionSummary.distance)
