...
class FirmwareWorkflowRuntimePolicyCommonTest {
    ...
    @Test
    fun firmwareFinalizationStepPlanningKeepsFilesystemAndSensorBranchesShared() {
        assertEquals(
            listOf("wait-for-device-update", "set-device-time", "stop-sync"),
            PolarWorkflowRuntimePlanning.firmwareFinalizationSteps(hasH10FileSystem = true, isDeviceSensor = true)
        )
        assertEquals(
            listOf("wait-for-device-update", "restore-backup", "set-device-time", "stop-sync"),
            PolarWorkflowRuntimePlanning.firmwareFinalizationSteps(hasH10FileSystem = false, isDeviceSensor = true)
        )
        assertEquals(
            listOf("wait-for-device-update", "restore-backup", "set-device-time", "restart-device", "wait-for-restart-reconnect"),
            PolarWorkflowRuntimePlanning.firmwareFinalizationSteps(hasH10FileSystem = false, isDeviceSensor = false)
        )
    }
