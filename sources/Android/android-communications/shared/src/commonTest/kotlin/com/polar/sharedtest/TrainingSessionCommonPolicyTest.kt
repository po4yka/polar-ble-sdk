...
class TrainingSessionCommonPolicyTest {
    ...
    fun trainingSessionSelectedPayloadParserOwnershipKeepsGeneratedModelBoundaryExplicit() {
        ...
        assertEquals("This neutral vector does not embed protobuf bytes; it turns the existing Android and iOS payload-read tests into a shared ownership contract for request ordering, progress, parsed component presence, malformed component isolation, unknown advanced sample-list handling, and decoded-payload reconstruction-slot planning with neutral session-summary scalar fields carried beside the bytes for platform adapters. payload-parser-policy.json now pins parser-family ownership; gzip payload decompression, selected protobuf field parsing, malformed payload isolation, deterministic public-model read planning, neutral session-summary scalar DTO assembly, and neutral reconstruction planning are owned by shared, while generated public protobuf object reconstruction remains platform-owned in Android and iOS adapters. Rollback for this parser-family slice is to ignore the neutral reconstruction fields/bridge columns and continue platform parsing from decoded payload bytes without moving generated public model construction into common.", vector.stringValue("notes"), vector.stringValue("id"))
    }
    ...
    fun trainingSessionPayloadParserGoldenVectorDefinesExecutableCommonParserOwnershipPolicy() {
        ...
        assertEquals("Selected training payload protobuf field parsing now executes in shared for these parser cases; generated public protobuf object construction remains platform-owned while neutral reconstruction planning maps decoded payload bytes to Android and iOS adapters. Gzip decompression and public-model slot planning are shared production code, and this vector remains the shared parser ownership contract consumed by commonTest and pinned by Android/iOS byte-level characterization tests.", expected.stringValue("commonDecision"), vector.stringValue("id"))
    ...
    @Test
    fun trainingSessionReadinessManifestNamesEverySharedContractBehaviorFamily() {
    ...
    private companion object {
        ...
        const val TRAINING_SESSION_MISSING_EXERCISE_FILE_COMMON_DECISION = "Android currently returns a partial exercise when an exercise data file request fails; iOS currently propagates the request failure. Choose an explicit shared policy before moving training-session read orchestration to shared."
